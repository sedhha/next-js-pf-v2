...
const MobileMenu = ({ currentPath }: MobileMenuProps) => {
    ...
    return (
        <div className="md:hidden">
            ...
            >
                <span className={`hamburger-line-1 block w-5 h-0.5 bg-white transition-all duration-200 ${isOpen ? 'transform rotate-45 translate-y-1.5' : ''}`}></span>
                <span className={`hamburger-line-2 block w-5 h-0.5 bg-white mt-1 transition-all duration-200 ${isOpen ? 'opacity-0' : ''}`}></span>
                <span className={`hamburger-line-3 block w-5 h-0.5 bg-white mt-1 transition-all duration-200 ${isOpen ? 'transform -rotate-45 -translate-y-1.5' : ''}`}></span>
            </label>

            {/* Mobile menu dropdown - works with both CSS-only and JS */}
            <div className={`mobile-menu-dropdown fixed top-20 left-0 right-0 z-40 background-transition-all duration-200 backdrop-blur-md border-b border-white/10 opacity-0 pointer-events-none peer-checked:opacity-100 peer-checked:pointer-events-auto ${isOpen ? '!opacity-100 !pointer-events-auto' : ''
                }`}>
